...
export default defineEventHandler(async (event) => {
  ...
  const { fileName, fileSize, mimeType, amount, label } = await readBody(event)
  ...
  if (amount === undefined || amount === null || amount === '' || Number.isNaN(Number(amount))) {
    throw createError({ statusCode: 400, statusMessage: 'amount requis (montant de la facture en EUR)' })
  }
