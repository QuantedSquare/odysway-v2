...
export function RequiredProgress(props: ProgressProps) {
  const {members} = props
  ...
  if(totalCount !== 5) return <Stack space={4}>{props.renderDefault(props)}</Stack>
    ...
    <Stack space={4}>
      <Card tone={isComplete ? `positive` : `transparent`} border padding={3} radius={2}>
        <Flex align="center" gap={3}>
          ...
          <Box>
            {completeCount} / {totalCount} {completeCount !== totalCount ? 'Champs requis' : 'OK ! Vous pouvez publier votre voyage'}
          </Box>
        </Flex>
      </Card>
