<template>
  <v-row>
    ...
    <v-col v-for="deal in dealsLastMinute" :key="`lastminute-${deal.slug}-${deal.dates[0]?.departure_date}`" cols="12"
      sm="6" md="4">
    ...
    <template v-for="monthName in paginatedMonthNames" :key="monthName">
      ...
      <v-col v-for="(deal, i) in dealsToDisplayInMonth(monthName)" v-else :key="`${monthName}-${deal.slug}-${i}`"
        cols="12" sm="6" md="4">
        <VoyageCard :voyage="deal" />
      </v-col>
    </template>
    ...
    <v-col v-if="nbPages > 1" cols="12">
      <v-pagination v-model="pagination.currentPage" :length="nbPages" :total-visible="5" variant="flat"
        density="comfortable" rounded="circle" active-color="primary" elevation="3" class="my-4"
        @click="goTo(scrollTarget, { offset: -50 })" @next="pagination.currentPage++"
