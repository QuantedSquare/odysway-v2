...
const travelDates_get = defineEventHandler(async (event) => {
  ...
  return data.map((date) => {
    ...
    return {
      ...
      closing_date: ge(date.departure_date).subtract((_e = (_d = metaBySlug[date.travel_slug]) == null ? void 0 : _d.closingDays) != null ? _e : 30, "day").toDate(),
      departure_date: new Date(date.departure_date),
      early_bird: ge().isAfter(ge(date.departure_date).add(7, "month")) ? date.early_bird : false,
      last_minute: ge(date.departure_date).diff(ge(), "day") <= 31 ? date.last_minute : false
    };
  });
});
