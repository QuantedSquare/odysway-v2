...
const _assets = {
  ["nuxt-og-image:fonts:Inter-normal-400.ttf.base64"]: {
    ...
    meta: {"type":"text/plain; charset=utf-8","etag":"\"652cc-qEeSD1DXCSV8gPP2rnBA6ePGdZ4\"","mtime":"2026-04-14T18:22:14.885Z"}
  },
  ["nuxt-og-image:fonts:Inter-normal-700.ttf.base64"]: {
    import: () => import('../raw/Inter-normal-700.ttf.mjs').then(r => r.default || r),
    meta: {"type":"text/plain; charset=utf-8","etag":"\"674f0-FZReUXHhPTnY0HmYVn2iPpKm9ds\"","mtime":"2026-04-14T18:22:14.885Z"}
  }
};
...
const _inlineRuntimeConfig = {
  "app": {
    "baseURL": "/",
    "buildId": "f351e989-223d-4d05-ba2d-c2c34afdd178",
  ...
  "sanity": {
    "visualEditing": {
      "previewModeId": "25ad93282582ebd5715287d10140942b",
...
function formatDate(orderDate) {
  if (orderDate) {
    return xe(orderDate, "DD/MM/YYYY").format("YYYY-MM-DD");
  } else {
    return "";
  }
}
...
const upsertContactIntoSupabase = async (contactId) => {
  ...
  try {
    ...
    const contactToUpsert = {
      ...
      birthdate: ((_a = acContact.contact.fieldValues.find((i) => i.field === "1")) == null ? void 0 : _a.value) ? xe(acContact.contact.fieldValues.find((i) => i.field === "1").value, "YYYY-MM-DD").toISOString() : null,
...
const createDepartureDeal = async ({ travelDate, travelTitle, stageId, totalValue = 0, totalNbTravelers = 0, linkBms = "", travelType = "Voyage de Groupe" }) => {
  const departureFormatted = xe(travelDate.departure_date).format("DD/MM");
  const returnFormatted = xe(travelDate.return_date).format("DD/MM/YYYY");
...
const computeDepartureStage = (departureDate, returnDate, bookedSeat, minTravelers) => {
  const today = xe();
  const departure = xe(departureDate);
  const returnDay = xe(returnDate);
...
const createDataQuote = (data, insuranceType) => {
  ...
  const dataQuote = {
    ...
    nombre: nbTravelers,
    DATE_RESA: xe().format("DD/MM/YYYY"),
    depart: xe(departureDate, "YYYY-MM-DD").format("DD/MM/YYYY"),
    retour: xe(returnDate, "YYYY-MM-DD").format("DD/MM/YYYY"),
    devise: "EUR",
...
const quote = async (body) => {
  const timestampNow = xe().valueOf();
  const timestampDepart = xe(body.departureDate).valueOf();
...
const notify = async (paymentSession, insuranceItem, dealCustomFields, client, isCapExploraction) => {
  ...
  const data = {
    ...
    depart: xe(dealCustomFields.departureDate).format("DD/MM/YYYY"),
    retour: xe(dealCustomFields.returnDate).format("DD/MM/YYYY"),
    devise: "EUR"
  };
...
const _AmfJYT = lazyEventHandler(() => {
...
const handlers = [
  ...
  { route: '/_ipx/**', handler: _AmfJYT, lazy: false, middleware: false, method: undefined },
