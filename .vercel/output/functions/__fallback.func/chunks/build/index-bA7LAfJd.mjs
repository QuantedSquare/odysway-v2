...
`,u={__name:"index",__ssrInlineRender:true,async setup(s){let i,o;const l=up(),{data:e}=([i,o]=withAsyncContext(async()=>Qc("categories-with-voyages",async()=>{try{return await l.fetch(V)||[]}catch{return []}})),i=await i,o(),i),c=computed(()=>({items:e.value?.categories?.map(t=>({id:t._id,title:t.title,slug:t.slug?.current,image:t.image,type:"thematiques",discoveryTitle:t.discoveryTitle||t.description||""})).filter(t=>t.image?.asset?._ref),selectedItem:null,pageTitle:e.value?.pageContent?.index?.pageTitle||"Toutes nos thématiques",showOnBottom:false}));return pt({htmlAttrs:{lang:"fr"}}),e.value?.pageContent&&K({seoData:e.value?.pages?.seo,content:e.value?.pages,pageType:"website",slug:"thematiques"}),(t,g,_,d)=>{const y=ae,p=Gt;g(ssrRenderComponent(y,mergeProps({"page-content":unref(e).pageContent,"displayed-data":unref(c),type:"thematiques"},d),{content:withCtx((I,a,f,v)=>{if(a)a(ssrRenderComponent(p,{voyages:unref(e).categories},null,f,v));else return [createVNode(p,{voyages:unref(e).categories},null,8,["voyages"])]}),_:1},_));}}},n=u.setup;u.setup=(s,i)=>{const o=useSSRContext();return (o.modules||(o.modules=new Set)).add("pages/thematiques/index.vue"),n?n(s,i):void 0};

export { u as default };
//# sourceMappingURL=index-bA7LAfJd.mjs.map
