import { s as se } from './SearchHeroSection-bzkK_7Va.mjs';
import { Q as Qt, G as Gt } from './SearchField-C1No64IL.mjs';
...
import { _ as Lu, b as gl, h as Er, a0 as cn, k as Ft, b8 as vn, d as pt, u as lp, a as Jc, V as fi, e as Zn, f as lt, aK as Jn, a1 as gi, U as we } from './server.mjs';
import O from 'groq';
import { H } from './VCheckbox-CCkh8DFz.mjs';
...
const oe={__name:"index",__ssrInlineRender:true,async setup(ce){let l,y;const{lgAndUp:V}=gl(),{trackSearchBar:de,trackViewItemList:ve}=Er(),{formatVoyagesForGtm:he}=cn();Ft({meta:[{name:"html-attrs",content:"lang=fr"},{name:"robots",content:"noindex, follow"},{name:"canonical",content:"https://www.odysway.com/voyages"}]});const ue=vn(),n=pt(),o$1=computed(()=>n.query),d=ref(n.query.confirmed==="true"),F=lp(),_e=O`*[_type == "search"][0]{
  oneTrip,
  multipleTrips,
  resetButton,
  image,
  searchHero
}`,{data:g}=([l,y]=withAsyncContext(()=>Jc("search-content-page",()=>F.fetch(_e))),l=await l,y(),l),{data:ie}=Jc("fetchedDestination",()=>{if(n.query.destination){const t=O`*[_type == "destination" && slug.current == $slug][0]{
      title,
      interjection,
      image
...
}`,{data:Q}=([l,y]=withAsyncContext(()=>Jc("destinations",()=>F.fetch(qe))),l=await l,y(),l),{data:ze}=([l,y]=withAsyncContext(()=>Jc("travels-by-date",()=>$fetch("/api/v1/booking/travels-by-date"))),l=await l,y(),l),{data:Ce}=([l,y]=withAsyncContext(async()=>Jc(`search-${JSON.stringify(n.query)}`,async()=>{let t=null,r=null,i=false,c=false;if(n.query.destination){const b=I.value.find(k=>k.slug===n.query.destination);if(b)i=true,r=b.slug;else if(n.query.destination==="top-destination")i=true,c=true;else {const k=Q.value.find(f=>f.slug===n.query.destination);k&&(t=k.title);}}const m=n.query.travelType||null,_=n.query.from||null,S=O`*[
      _type == "voyage" && (
...
//# sourceMappingURL=index-DsrO8-Rc.mjs.map
